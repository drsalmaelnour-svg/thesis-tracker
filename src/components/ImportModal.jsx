...
// Map your exact CSV columns to our DB fields
function mapRow(row) {
  return {
    ...
    co_supervisor:    row['Co-Supervisor']       || row[' Co-Supervisor'] || '',
    co_sup_email:     row['Co-supervisor Email'] || row[' Co-supervisor Email'] || '',
  }
}
        ...
        <div className="flex items-center justify-between p-5 border-b border-navy-700/50">
          <div>
            ...
            <p className="text-xs text-navy-400 mt-0.5">
              Supports your format: Reg No, Student Name, Student Email, Supervisor, Supervisor Email
            </p>
          </div>
            ...
            <div className="space-y-5">
              ...
              <div className="bg-navy-800/40 rounded-xl p-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {['Reg No', 'Student Name', 'Student Email', 'Supervisor', 'Supervisor Email', 'Co-Supervisor', 'Co-supervisor Email'].map(col => (
                    <span key={col} className="bg-navy-700/60 text-navy-300 text-xs px-2 py-1 rounded-lg font-mono">{col}</span>
                  ))}
                </div>
                <p className="text-xs text-navy-500 mt-2">Only Student Name and Student Email are required. All other columns are optional.</p>
              </div>
            </div>
          )}
          ...
          {step === 'preview' && (
            <div className="space-y-4">
              ...
              <p className="text-xs text-navy-400">
                Supervisors will be created automatically if they don't exist yet.
              </p>
            </div>
          )}
          ...
          {step === 'done' && (
            <div className="text-center py-8 space-y-4">
              ...
              <div className="flex justify-center gap-4">
                <div className="bg-emerald-900/20 border border-emerald-700/40 rounded-xl px-6 py-3">
                  <p className="text-2xl font-bold text-emerald-300">{results.success}</p>
                  <p className="text-xs text-emerald-400">Imported</p>
                </div>
                {results.failed > 0 && (
                  <div className="bg-red-900/20 border border-red-700/40 rounded-xl px-6 py-3">
                    <p className="text-2xl font-bold text-red-300">{results.failed}</p>
                    <p className="text-xs text-red-400">Failed (duplicates)</p>
                  </div>
                )}
              </div>
            </div>
