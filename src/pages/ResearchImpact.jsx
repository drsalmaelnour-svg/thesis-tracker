...
const IMPACT_CRITERIA = [
  {
    key:   'has_publication',
    label: 'Published in a peer-reviewed journal or indexed in SCOPUS',
    hint:  'Upload your publication, acceptance letter, or DOI confirmation',
    type:  'Publication',
  },
  {
    key:   'has_ip',
    label: 'Intellectual Property developed (patent, copyright, trademark)',
    hint:  'Upload your IP certificate or registration document',
    type:  'Intellectual Property',
  },
  {
    key:   'has_industry_partner',
    label: 'Research supported by an industry partner',
    hint:  'Upload the MOU, agreement, or letter of support',
    type:  'Industry Partnership',
    extra: 'partner_name',
  },
  {
    key:   'has_public_events',
    label: 'Presented at 5 or more public events (conferences, symposiums, lectures)',
    hint:  'Upload invitation letters, programmes, or certificates of participation',
    type:  'Public Events',
  },
  {
    key:   'has_policy_citation',
    label: 'Research cited in a government or policy document',
    hint:  'Upload or link the policy document referencing your research',
    type:  'Policy Citation',
  },
  {
    key:   'has_commercialisation',
    label: 'Revenue generated from research output (≥ AED 50,000)',
    hint:  'Upload financial records or commercialisation agreement',
...
export default function ResearchImpact() {
  ...
  const [files,     setFiles]     = useState({})     // key: impactType → { file, uploading, done, url, fileName }
  const [partnerName, setPartnerName] = useState('')
  const [comments,  setComments]  = useState('')
  const [noImpact,  setNoImpact]  = useState(false)

  // Load student from token
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const { data, error: err } = await supabase
          .from('students')
          .select('*, departments(name,primary_color,accent_color)')
          .eq('impact_token', token)
          .single()
  ...
  async function uploadFile(impactType, file) {
    ...
    try {
      ...
      setFiles(prev => ({
        ...prev,
        [impactType]: { file, uploading: false, done: true, url: data.viewUrl, fileName: data.fileName }
      }))
    } catch(e) {
      setFiles(prev => ({
        ...prev,
        [impactType]: { file, uploading: false, done: false, error: e.message, fileName: file.name }
      }))
    }
  }
  ...
  async function handleSubmit() {
    ...
    // Check all checked criteria have evidence uploaded
    for (const c of IMPACT_CRITERIA) {
      if (checks[c.key] && (!files[c.type] || !files[c.type].done)) {
    ...
    try {
      ...
      const evidence_files = IMPACT_CRITERIA
        ...
        .map(c => ({
          impactType: c.type,
          fileName:   files[c.type].fileName,
          fileUrl:    files[c.type].url,
        }))
          ...
          <>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-5">
              <p className="text-sm text-gray-600 leading-relaxed">
                ...
                This information contributes to the university's research KPI reporting.
              </p>
            </div>
                ...
                <div key={criterion.key}
                  ...
                  <button
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">{criterion.hint}</p>
                    </div>
                  </button>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-4">
              ...
              <textarea
                className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none resize-none focus:border-amber-400"
                rows={3}
                placeholder="Any additional context about your research impact…"
                value={comments}
