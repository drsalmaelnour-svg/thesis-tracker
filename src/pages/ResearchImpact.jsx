  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (existingImpact) {
          // Check if student already submitted (not just supervisor)
          if (existingImpact.submitted_at && !existingImpact.supervisor_submitted_at) {
            // Student submitted, supervisor hasn't — show as submitted
            setSubmitted(true)
            return
          }
          if (existingImpact.submitted_at && existingImpact.supervisor_submitted_at) {
          ...
          // Supervisor submitted first — pre-fill student form with supervisor's data
          // but mark supervisor-filled fields as locked
          if (existingImpact.supervisor_submitted_at && !existingImpact.submitted_at) {
            const preChecks = {}
            IMPACT_CRITERIA.forEach(c => {
              if (existingImpact[c.supKey]) preChecks[c.key] = true
            })
            setChecks(preChecks)
